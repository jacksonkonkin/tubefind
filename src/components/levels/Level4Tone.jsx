...
import { toneOptions } from '../../data/filterOptions';
...
export default function Level4Tone() {
  const { selections, setSelection, nextLevel } = useFilter();
  const selected = selections.level_4;
  const [dismissed, setDismissed] = useState(false);

  const suggestion = useMemo(() => getSuggestion(4, selections), [selections.level_1, selections.level_2, selections.level_3]);
  ...
  const suggestedOption = showSuggestion
    ? toneOptions.find((o) => o.id === suggestion)
    : null;
  ...
  return (
    <LevelShell
      level={4}
      title="What vibe?"
      subtitle="Set the tone and energy"
    >
